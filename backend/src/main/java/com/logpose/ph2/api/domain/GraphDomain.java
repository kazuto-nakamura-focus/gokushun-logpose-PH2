...
// * Fステージデータの累積値に達しているモデルデータの日付を戻り値のオブジェクトに追加する
		...
		for (Ph2RealGrowthFStageEntity item : fstages)
			...
			if (null != targetData)
				{
				localDate1 = targetData.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
				}
			// 実績テーブルの累積F値
			...
			for (; dayIndex < values.size(); dayIndex++)
				...
				if (null != localDate1)
					{
					LocalDate localDate2 = value.getDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
